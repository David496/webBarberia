$(function(){
    ...
    let form = $('#formTablaReportes');
    let url = form.attr('action');

    $("#tablaReportesId").DataTable({
        language: {
...
const registrarReporte = () => {
    //obteniendo la url
    const form = $("#formAgregarReporteId");
    const url = form.attr('action');

    $('#formAgregarReporteId').off('submit').on('submit', function(e) {
        e.preventDefault();
        const dataForm = new FormData($('#formAgregarReporteId')[0]);

        Swal.fire({
            title: "¿Confirma continuar?",
            text: "¿Confirma Registrar el Reporte?",
            icon: "question",
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    $('#tablaReportesId').DataTable().ajax.reload(null, false);
                    $("#modalAgregarReporteId").modal("hide");
                    $("#formAgregarReporteId").trigger("reset");
                });
            }
        })
    });
...
const eliminarReporte = (id) => {
    ...
    $("#reporteEliminarId").val(id);
    //obteniendo la url
    const form = $("#formEliminarReporteId");
    const url = form.attr('action');
    const dataForm = new FormData($('#formEliminarReporteId')[0]);
        Swal.fire({
            title: "¿Confirma continuar?",
            text: "¿Confirma eliminar Reporte?",
            icon: "question",
            showCancelButton: true,
            confirmButtonClass: "btn btn-success w-xs me-2 mt-2",
            cancelButtonClass: "btn btn-light w-xs mt-2",
