...
const infoProducto = (id) => {
    ...
    $.get(urlGetWithId, (response) => {
        let {producto:producto} = response;
        $('#nombreProducto_info_id').val(producto.nombre_producto);
        $('#stock_actual_id').val(producto.stock);
        $('#stock_mas_id').val(0);
    }).done(() => {
        console.log("ok");
    })
...
const aumentarStock = (id) => {
    $('#formAumentarStockId').trigger("reset");
    let modalActualiza = $('#modalAumentarStockId')
    modalActualiza.modal("show");
    $('#getProductId').val(id);
    infoProducto(id);

    //obteniendo la url
    const form = $("#formAumentarStockId");
    const url = form.attr('action');

    $('#formAumentarStockId').off('submit').on('submit', function(e) {
        e.preventDefault();
        const dataForm = new FormData($('#formAumentarStockId')[0]);
